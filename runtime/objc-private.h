...
#   define ISA_MAGIC_VALUE 0x001d800000000001ULL
    struct {
         ...
         */
        uintptr_t nonpointer        : 1;  ///< 0 raw isa ,1 nonpointer isa，存储其他信息，从iOS7以后，这个值为1，即ISA指针除了存放类信息以外，还存放has_assoc、has_cxx_dtor信息等
        uintptr_t has_assoc         : 1;  ///是否曾经或正在被关联引用,如果没有，可以被快速释放内存
        uintptr_t has_cxx_dtor      : 1;  ///< 是否有C++或ARC析构器，如果没有，可以快速释放内存
        uintptr_t shiftcls          : 44; // MACH_VM_MAX_ADDRESS 0x7fffffe00000              存储类信息，即raw isa
        uintptr_t magic             : 6;
        ...
        uintptr_t extra_rc          : 8;  ///< 对象的引用计数超过1，比如extra_rc = 5,则retainCount=6

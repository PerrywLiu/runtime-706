...
#   define POOL_BOUNDARY nil
...
#if PROTECT_AUTORELEASEPOOL //0 代码不会执行
    ...
    id *next;                ///< 指向内存结构中为空的地址，如果当前指向被赋予值，则指向下一个为空的地址
    pthread_t const thread;  ///< 当前页所在的线程
    AutoreleasePoolPage * const parent;   //内部构造为双向链表结构 parent和child,链表可以通过栈和队列实现，由push和pop方法可见，AutoReleasePool是通过栈实现的
    ...
    id *add(id obj)
    {
        ...
        *next++ = obj; ///< 相当于 *next = obj; next++; 是一个压栈操作,将对象加入栈中，然后移动栈顶指针
        protect();
        return ret;
    }
    ...
    static AutoreleasePoolPage *pageForPointer(uintptr_t p) 
    {
        ...
        uintptr_t offset = p % SIZE; //将指针与页面大小(4096)取余，得到当前指针的偏移量.因为所有的AutoReleasepoolPage在内存中都是对齐的

        assert(offset >= sizeof(AutoreleasePoolPage));

        result = (AutoreleasePoolPage *)(p - offset);
        result->fastcheck();//检查result 是不是一个AutoReleasePoolPage

...
struct protocol_t : objc_object {
    const char *mangledName;           /// 来源于C++的命名重整技术，在C++里面是用来区别重载时的函数
    struct protocol_list_t *protocols;   /// 此协议遵循的协议
    method_list_t *instanceMethods;
    method_list_t *classMethods;
    method_list_t *optionalInstanceMethods;
    method_list_t *optionalClassMethods;
    property_list_t *instanceProperties;    /// 实例属性 为什么不区分可选与否？
    uint32_t size;   // sizeof(protocol_t)
    uint32_t flags;
    // Fields below this point are not always present on disk.
    const char **_extendedMethodTypes;
    const char *_demangledName;      /// 来源于C++的命名重整技术，在C++里面是用来区别重载时的函数
    property_list_t *_classProperties;     /// 类属性 与类方法一样，通过类名调用 举例@property (class, nonatomic, copy) NSString *name;
...
//提供了运行时对类扩展的能力
//rw- read and write 可读写 存放运行时添加的信息
struct class_rw_t {
    // Be warned that Symbolication knows the layout of this structure.
    uint32_t flags;
    uint32_t version;

    //ro-read only  只读结构体 存放在编译期已经确定的信息
    const class_ro_t *ro;
...
struct objc_class : objc_object {
    // Class ISA;
    Class superclass;           ///< 8字节长度 ？
    cache_t cache;             // formerly cache pointer and vtable 虚表        16字节 ？
    class_data_bits_t bits;    // class_rw_t * plus custom rr/alloc flags  rr/aclloc标记是指含有这些方法：retainCount/retain/release/autorelease/alloc

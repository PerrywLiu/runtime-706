...
static Class realizeClass(Class cls)
{
    ...
    //cls->data class_rw_t 此时为 class_ro_t，在这里明确为ro类型
    ...
    if (ro->flags & RO_FUTURE) {
        ...
        rw->ro = ro;    ///< 把ro赋值给 rw中的ro字段
        rw->flags = RW_REALIZED|RW_REALIZING;
        cls->setData(rw); ///< 把rw赋值回去，这一步完成之后，rw和ro就被正确设置了，但是rw中的方法、属性、协议列表还是空的。
    }
...
// SUPPORT_NONPOINTER_ISA
    ...
    // Attach categories
    // 把ro中的方法、属性、协议复制到rw中，另外会把此类category中所有的方法、属性、协议也拷贝进去.
    // OC之所以能在运行时做很多事情，都是基于runtime的这些支持
    methodizeClass(cls);
...
* already tried that.
**********************************************************************/
//在调用该方法之前，已经从缓存中查找过，没有命中缓存，所以这里就不需要进行缓存查找了。

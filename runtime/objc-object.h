...
// not SUPPORT_TAGGED_POINTERS
    ...
    //内联函数，在头文件中定义，代替C语言中表达式形式的宏定义，编译期执行
   /* C中使用define这种形式宏定义的原因是因为，C语言是一个效率很高的语言，这种宏定义在形式及使用上像一个函数，但它使用预处理器实现，没有了参数压栈，代码生成等一系列的操作,因此，效率很高，这是它在C中被使用的一个主要原因。
    *这种宏定义在形式上类似于一个函数，但在使用它时，仅仅只是做预处理器符号表中的简单替换，因此它不能进行参数有效性的检测，也就不能享受C++编译器严格类型检查的好处，另外它的返回值也不能被强制转换为可转换的合适的类型，这样，它的*使用就存在着一系列的隐患和局限性。
...
{ 
    ...
    if (!nonpointer) {
        ...
        //将cls右移三位存储存到shiftcls中，从isa_t的结构体重可以看到，低三位都是用来存储其他信息的，既然可以右移三位，那就代表类地址的低三位都是0，否则就出错了，补0的作用就是为了字节对齐
